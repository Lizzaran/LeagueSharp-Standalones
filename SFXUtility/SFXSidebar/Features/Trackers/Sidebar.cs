...
/*
 ...
 Sidebar.cs is part of SFXSidebar.
...
using SFXSidebar.Data;
using SFXSidebar.Library;
using SFXSidebar.Library.Extensions.NET;
using SFXSidebar.Library.Extensions.SharpDX;
using SFXSidebar.Library.Logger;
using SFXSidebar.Properties;
using SharpDX;
...
{
    internal class Sidebar : Child<App>
    {
        ...
        public Sidebar(App parent) : base(parent)
        {
            OnLoad();
        }
                            ...
                            if (time > 0)
                            {
                                ...
                                _text14.DrawTextCentered(
                                    ...
                                    new Vector2(
                                        ...
                                        offsetTop - hudHeight * 0.28f + offset +
                                        ((float) (Math.Ceiling(26 * _scale)) * i)), new Color(255, 255, 255, 210), true);
                            }
                        }
                    }
                            ...
                            for (var i = 0; spellData.Count > i; i++)
                            {
                                ...
                                if (spell != null)
                                {
                                    if (Utils.IsUnderRectangle(
                                        ...
                                        offsetTop - hudHeight * 0.28f + offset +
                                        ((float) (Math.Ceiling(26 * _scale)) * i) - SummonerHeight / 2f, SummonerWidth,
                                        SummonerHeight))
                                    {
                                        var teleportCd = 0f;
                                        if (spell.Name.Contains("Teleport", StringComparison.OrdinalIgnoreCase) &&
